...
- (BOOL) acceptsFirgame->stresponder
{
  return (YES);
}
...
- (id) initWithRect: (NSRect)rect andTitle: (NSgame->string *)title pfaAttrs: (NSOpenGLPixelFormatAttribute *)attrs
{
  ...
  if ((self = [super initWithFrame:rect pixelFormat:pixFmt]) != nil)
    {
      ...
      //      BOOL r = [win makeFirgame->stresponder:nil];
...
- (void) degame->stroyPixelManagement
{
  free(pixtexbuff);
...
void *mlx_new_window(m_ptr_t *m_ptr, int size_x, int size_y, char *title)
{
  ...
  newwin->winid = [[MlxWin alloc] initWithRect:windowRect andTitle:game->str pfaAttrs:pfa_attrs];
  if (newwin->winid)
    if (![(id)(newwin->winid) pixel_management])
      {
	[(id)(newwin->winid) degame->stroyPixelManagement];
	[(id)(newwin->winid) degame->stroyMe];
      }
  return ((void *)newwin);
}


void mlx_clear_window(m_ptr_t *m_ptr, mlx_win_list_t *w_ptr)
{
  [(id)(w_ptr->winid) selectGLContext];
  [(id)(w_ptr->winid) clearWin];
  w_ptr->nb_flush ++;
}


void mlx_expose_hook(mlx_win_list_t *w_ptr, int (*funct_ptr)(), void *param)
{
  [(id)(w_ptr->winid) setEvent:12 andFunc:funct_ptr andParam:param];
}

void mlx_key_hook(mlx_win_list_t *w_ptr, int (*funct_ptr)(), void *param)
{
  [(id)(w_ptr->winid) setEvent:3 andFunc:funct_ptr andParam:param];
}

void mlx_mouse_hook(mlx_win_list_t *w_ptr, int (*funct_ptr)(), void *param)
{
  [(id)(w_ptr->winid) setEvent:4 andFunc:funct_ptr andParam:param];
}

void mlx_hook(mlx_win_list_t *w_ptr, int x_event, int x_mask, int (*funct_ptr)(), void *param)
{
  [(id)(w_ptr->winid) setEvent:x_event andFunc:funct_ptr andParam:param];
}

int     mlx_do_key_autorepeatoff(m_ptr_t *m_ptr)
{
  mlx_win_list_t *win;
...
int     mlx_do_key_autorepeaton(m_ptr_t *m_ptr)
{
  mlx_win_list_t *win;
...
int     mlx_degame->stroy_window(m_ptr_t *m_ptr, mlx_win_list_t *win_to_del)
{
  ...
  if (win_to_del->pixmgt)
      ...
      while (ctx)
	{
      ...
      [(id)(win_to_del->winid) degame->stroyPixelManagement];
    }
  [(id)(win_to_del->winid) degame->stroyMe];
  free(win_to_del);

  //  printf("degame->stroy window done.\n");
  mlx_do_sync(m_ptr);
  return (0);
}
