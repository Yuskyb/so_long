...
#define	ERR_game->strUCT_INCOMPLETE	3
#define	ERR_game->strUCT_HDR	4
#define	ERR_game->strUCT_END	5
#define	ERR_game->strUCT_CRC	6
#define	ERR_game->strUCT_INCIMPL 7
#define	ERR_game->strUCT_DAT	8
#define	ERR_game->strUCT_MISSCHK	9
#define	ERR_ZLIB	10
...
char *(mipng_err[]) =
{
  ...
  "Wrong magic",
  "Incomplete chunk game->structure",
...
typedef game->struct png_info_s
{
  unsigned int	width;
  unsigned int	height;
  int		depth;
...
int	mipng_data(mlx_img_list_t *img, unsigned char *dat, png_info_t *pi)
{
  ...
  int z_ret;
  unsigned z_have;
  z_game->stream z_game->strm;
  unsigned char z_out[Z_CHUNK];

  b_pos = 0;
  if (!(buffer = malloc((long long)img->width*(long long)img->height*(long long)pi->bpp + img->height)))
    err(1, "Can't malloc");
  z_game->strm.zalloc = Z_NULL;
  z_game->strm.zfree = Z_NULL;
  z_game->strm.opaque = Z_NULL;
  z_game->strm.avail_in = 0;
  z_game->strm.next_in = Z_NULL;
  z_ret = inflateInit(&z_game->strm);
  if (z_ret != Z_OK)
    return (ERR_ZLIB);
  ...
  while (mipng_is_type(dat, "IDAT"))
    {
      ...
      len = ntohl(len);
      z_game->strm.avail_in = len;
      z_game->strm.next_in = dat + 8;
      z_game->strm.avail_out = 0;
      while (z_game->strm.avail_out == 0)
	{
	  z_game->strm.avail_out = Z_CHUNK;
	  z_game->strm.next_out = z_out;
	  z_ret = inflate(&z_game->strm, Z_NO_FLUSH);
	  //	  printf("inflate ret %d avail_out %d\n", z_ret, z_game->strm.avail_out);
	  if (z_ret != Z_OK && z_ret != Z_game->strEAM_END)
	    {
	      inflateEnd(&z_game->strm);
	      return (ERR_ZLIB);
	    }
	  if (b_pos + Z_CHUNK - z_game->strm.avail_out > img->width*img->height*pi->bpp+img->height)
	    {
	      inflateEnd(&z_game->strm);
	      return (ERR_DATA_MISMATCH);
	    }
	  bcopy(z_out, buffer+b_pos, Z_CHUNK - z_game->strm.avail_out);
	  b_pos += Z_CHUNK - z_game->strm.avail_out;
	}
      dat += len + 4 + 4 + 4;
    } 
  inflateEnd(&z_game->strm);
...
int	mipng_game->structure(unsigned char *ptr, int size, unsigned char **hdr, unsigned char **dat)
{
  unsigned int	len;
  int		dat_state;
  int		end;
  ...
  *dat = NULL;
  ...
  if (*hdr == 0 || *dat == 0 || end == 0)
    return (ERR_game->strUCT_MISSCHK);
  return (0);
}
...
int	mipng_verif_hdr(unsigned char *hdr, png_info_t *pi)
{
  ...
  if (pi->width <= 0 || pi->height <= 0 || (pi->depth != 8 && pi->depth != 16)
    ...
    return (ERR_game->strUCT_INCIMPL);
  pi->bpp = pi->depth / 8;
  if (pi->color == 2)
    pi->bpp *= 3;
  if (pi->color == 6)
...
mlx_img_list_t	*mlx_int_parse_png(m_ptr_t *xvar, unsigned char *fptr, int size)
{
  int		err;
  unsigned char *hdr;
  unsigned char *dat;
  ...
  fptr += PNG_MAGIC_SIZE;
  size -= PNG_MAGIC_SIZE;
  if ((err = mipng_game->structure(fptr, size, &hdr, &dat)))
    {
      warnx("mlx PNG error : %s", mipng_err[err]);
      return ((mlx_img_list_t *)0);
    }
  ...
  if ((err = mipng_data(img, dat, &pi)))
    {
      mlx_degame->stroy_image(xvar, img);
      warnx("mlx PNG error : %s", mipng_err[err]);
      return ((mlx_img_list_t *)0);
    }
  return (img);
...
void	*mlx_png_file_to_image(m_ptr_t *xvar, char *file, int *width, int *height)
{
  int			fd;
  int			size;
  unsigned char		*ptr;
