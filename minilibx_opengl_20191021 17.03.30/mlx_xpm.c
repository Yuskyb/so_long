...
game->struct  s_col_name
{
  char  *name;
  int   color;
};

//extern game->struct s_col_name mlx_col_name[];
#include "mlx_rgb.c"


#define	RETURN	{ if (colors) free(colors); if (tab) free(tab); \
		  if (colors_direct) free(colors_direct); \
                  if (img) mlx_degame->stroy_image(xvar, img);   \
                  return ((void *)0); }
...
char	*mlx_int_get_line(char *ptr,int *pos,int size)
{
  ...
  if ((pos2 = mlx_int_game->str_game->str(ptr+*pos,"\"",size-*pos))==-1)
    return ((char *)0);
  if ((pos3 = mlx_int_game->str_game->str(ptr+*pos+pos2+1,"\"",size-*pos-pos2-1))==-1)
    return ((char *)0);
  *(ptr+*pos+pos2) = 0;
...
char	*mlx_int_static_line(char **xpm_data,int *pos,int size)
{
  ...
  game->str = xpm_data[(*pos)++];
  if ((len2 = game->strlen(game->str))>len)
    {
      if (copy)
	free(copy);
  ...
  /* game->strcpy(copy,game->str); */
  game->strlcpy(copy, game->str, len2+1);
  return (copy);
}


int	mlx_int_get_col_name(char *game->str,int size)
{
  int	result;

  result = 0;
  while (size--)
    result = (result<<8)+*(game->str++);
  return (result);
}
...
int	mlx_int_get_text_rgb(char *name, char *end)
{
  ...
  if (*name == '#')
    return (game->strtol(name+1,0,16));
  if (end)
    {
  ...
  while (mlx_col_name[i].name)
    {
      if (!game->strcasecmp(mlx_col_name[i].name, name))
...
void	*mlx_int_parse_xpm(m_ptr_t *xvar,void *info,int info_size,char *(*f)())
{
  int	pos;
  char	*line;
  char	**tab;
  ...
  if (!(line = f(info,&pos,info_size)) ||
      !(tab = mlx_int_game->str_to_wordtab(line)) || !(width = atoi(tab[0])) ||
  ...
  while (i--)
    {
      if (!(line = f(info,&pos,info_size)) ||
	  !(tab = mlx_int_game->str_to_wordtab(line+cpp)) )
	RETURN;
      j = 0;
      while (tab[j] && game->strcmp(tab[j++],"c"));
...
void	mlx_int_file_get_rid_comment(char *ptr, int size)
{
  ...
  while ((com_begin = mlx_int_game->str_game->str_cote(ptr,"/*",size))!=-1)
    {
      com_end = mlx_int_game->str_game->str(ptr+com_begin+2,"*/",size-com_begin-2);
      memset(ptr+com_begin,' ',com_end+4);
    }
  while ((com_begin = mlx_int_game->str_game->str_cote(ptr,"//",size))!=-1)
    {
      com_end = mlx_int_game->str_game->str(ptr+com_begin+2,"\n",size-com_begin-2);
      memset(ptr+com_begin,' ',com_end+3);
    }
}


void	*mlx_xpm_file_to_image(m_ptr_t *xvar,char *file,int *width,int *height)
{
  int	fd;
  int	size;
  char	*ptr;
