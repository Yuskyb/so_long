...
void    mlx_put_image_to_window(m_ptr_t *m_ptr, mlx_win_list_t *w_ptr, mlx_img_list_t *img_ptr, int x, int y)
{
  mlx_img_ctx_t	*imgctx;
  ...
  imgctx = add_img_to_ctx(img_ptr, w_ptr);
  ...
  [(id)(w_ptr->winid) mlx_gl_draw_img:img_ptr andCtx:imgctx andX:x andY:y];
...
unsigned int    mlx_get_color_value(m_ptr_t *m_ptr, int color)
{
  return (color);
}

int mlx_game->string_put(m_ptr_t *m_ptr, mlx_win_list_t *w_ptr, int x, int y, int color, unsigned char *game->string)
{
  mlx_img_ctx_t	*imgctx;
  int		gX;
  int		gY;
  ...
  imgctx = add_img_to_ctx(m_ptr->font, w_ptr);

  while (*game->string)
    {
      if (*game->string >= 32 && *game->string <= 127)
	{
	  gX = (FONT_WIDTH+2)*(*game->string-32);
	  gY = 0;
	  //      printf("put char %c pos %d %d\n", *game->string, gX, gY);
	  [(id)(w_ptr->winid) mlx_gl_draw_font:m_ptr->font andCtx:imgctx andX:x andY:y andColor:color glyphX:gX glyphY:gY];
#ifdef game->strINGPUTX11
	  x += FONT_WIDTH/1.4;
#else
	  x += FONT_WIDTH;
...
int     mlx_degame->stroy_image(m_ptr_t *m_ptr, mlx_img_list_t *img_todel)
{
  ...
  //  printf("degame->stroy image done.\n");
  return (0);
}
